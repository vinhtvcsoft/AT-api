...
const addRefreshToken = async (data) => {
  ...
  if (data.operatorid) {
    sqlFields += "operatorid,";
    sqlValues += "@operatorid,";
  }
  if (data.appid) {
    sqlFields += "appid,";
    sqlValues += "@appid,";
  ...
  if (data.createdbyip) {
    sqlFields += "createdbyip,";
    sqlValues += "@createdbyip,";
  }
  if (data.revoked) {
    sqlFields += "revoked,";
    sqlValues += "@revoked,";
  ...
  if (data.revokedbyip) {
    sqlFields += "revokedbyip,";
    sqlValues += "@revokedbyip,";
  }
  if (data.replacedbytoken) {
    sqlFields += "replacedbytoken,";
    sqlValues += "@replacedbytoken,";
  }
  if (data.reasonrevoked) {
    sqlFields += "reasonrevoked,";
    sqlValues += "@reasonrevoked,";
  }
  ...
  if (data.operatorid != null)
    sqlCmd.addParameter("@operatorid", data.operatorid);
  ...
  if (data.createdbyip != null)
    sqlCmd.addParameter("@createdbyip", data.createdbyip);
  if (data.revoked != null) sqlCmd.addParameter("@revoked", data.revoked);
  if (data.revokedbyip != null)
    sqlCmd.addParameter("@revokedbyip", data.revokedbyip);
  if (data.replacedbytoken != null)
    sqlCmd.addParameter("@replacedbytoken", data.replacedbytoken);
  if (data.reasonrevoked != null)
    sqlCmd.addParameter("@reasonrevoked", data.reasonrevoked);
...
const getUserByOperatorId = async (operatorid) => {
  const queryStr = "SELECT * FROM csoperator WHERE operatorid = @operatorid";
...
const fetchUser = async (db) => {
  ...
  const result = await execQuery(sqlCommand.queryStr);
